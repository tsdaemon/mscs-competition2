...
def generate_validation_folds(train, path_to_folds):
    ...
    def prepare_train(df, fold):
        ...
        df_temp = df.copy()
        temp = df[(df["listen_type"] == 1) & (df["n"] >= fold)].groupby(['user_id']).apply(lambda x: x.iloc[-fold])[
            "ts_listen"].reset_index()
        ...
        feather.write_dataframe(df_temp[df_temp["n"] >= fold].groupby(["user_id"]).apply(lambda x: x.iloc[-1]),
                                path_to_folds + "/test_{}.feather".format(i))
        feather.write_dataframe(df_temp[df_temp["n"] >= fold].groupby(["user_id"]).apply(lambda x: x.iloc[:-1]),
                                path_to_folds + "/train_{}.feather".format(i))
